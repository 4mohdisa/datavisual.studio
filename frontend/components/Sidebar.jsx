...
import { Telescope, Database, MoreHorizontal, Pencil, Trash2, Search, LayoutDashboard } from 'lucide-react';
...
export default function Sidebar({
  ...
  // Dashboards get their own section at the top (they're the core artifact);
  // research conversations keep the recency grouping below.
  const groups = useMemo(() => {
    const dashboards = filtered.filter((c) => c.mode === 'dashboard');
    const rest = filtered.filter((c) => c.mode !== 'dashboard');
    const out = {};
    for (const c of rest) {
      const g = dateGroup(c.created_at);
      (out[g] ||= []).push(c);
    }
    const dated = GROUP_ORDER.filter((g) => out[g]?.length).map((g) => [g, out[g]]);
    return dashboards.length ? [['Dashboards', dashboards], ...dated] : dated;
  }, [filtered]);
    ...
    <div className="w-[280px] shrink-0 flex flex-col h-screen bg-[var(--background)] border-r border-[var(--border)]">
      <div className="p-4 border-b border-[var(--border)]">
        ...
        <button
          ...
          title={newChatDisabled ? 'Start the current research first' : 'New research'}
        >
            ...
            return (
              ...
              >
                <div className="flex items-center gap-1.5">
                  ...
                  {conv.mode === 'dashboard' && !isRenaming ? (
                    <LayoutDashboard size={14} strokeWidth={1.5} className="shrink-0 text-[var(--muted)]" />
