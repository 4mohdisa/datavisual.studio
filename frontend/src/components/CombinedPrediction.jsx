...
function Cell({ value }) {
  if (value == null) return <span className="text-[var(--faint)]">—</span>;
  return <span className="tabular-nums">{value}%</span>;
}

export default function CombinedPrediction({ suite }) {
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={i} className="odd:bg-[var(--raised)]">
                <td className="px-3 py-2 border border-[var(--border)] text-[oklch(0.88_0_0)]">{r.entity}</td>
